...
class OutputCart extends Component {
  ...
  render() {
    ...
    if (!uploadSucceed) {
      ...
      progressBarPercent = (
        totalEncodingProgress / selectedResolutionLength || 0.01
      ).toFixed(0);
    }

    const sortedDataResolutions = [...dataResolutions].sort(
      (a, b) => a.id - b.id
    );
